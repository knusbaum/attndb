...
- **Filesystem tools** (`write_doc`/`delete_doc`/`read_doc`) — no deps; new
...
- **[multi-user]** In order:
  1. **Filesystem tools** `write_doc` / `delete_doc` / `read_doc` over the
...
- **[capture, capture half]** Once `write_doc`/`read_doc` exist, wire the
...
| `write_doc`/`delete_doc`/`read_doc` | multi-user | M2 | — |
