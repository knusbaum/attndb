...
// for the pooled passes.
func onnxEncoders(modelDir string) (core.MultiVectorEncoder, core.SingleVectorEncoder, error) {
	cb, err := onnx.NewColBERT(modelDir)
	if err != nil {
		return nil, nil, err
	}
	sv, err := onnx.NewSingleVector(modelDir + "/single")
	if err != nil {
		return nil, nil, err
	}
	return cb, sv, nil
